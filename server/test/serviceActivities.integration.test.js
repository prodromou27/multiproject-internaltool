...
test('managed reports follow a conflict-safe reviewed workflow with an audit trail',async () => {
  ...
  const reviewDownload=await fetch(`${baseUrl}/api/managed-customers/report-reviews/${report.id}/download`,{ headers:{ Authorization:`Bearer ${ids.tokenPlanner}`,'X-SolutionsHub-Request':'1' } });
  ...
  assert.equal((await fetch(`${baseUrl}/api/managed-customers/report-reviews/${report.id}/download`,{ headers:{ Authorization:`Bearer ${ids.tokenPlanner}`,'X-SolutionsHub-Request':'1' } })).status,404);
