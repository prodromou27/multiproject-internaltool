...
function buildDocument(model) {
  ...
  const builders={
    ...
    ticket_summary:() => { if (!model.tickets.enabled) return;const current=model.tickets.analytics.current,period=model.tickets.analytics.period;addTable(children,'Ticket Summary',['Measure','Value'],[['Open backlog',current.total_open],['Created during period',period.created],['Resolved during period',period.resolved],['Closed during period',period.closed],['Rejected during period',period.rejected],['SLA breaches during period',period.sla_breaches]]);addTable(children,'Open Tickets by Status',['Status','Count'],current.statuses.map(row => [row.name,row.count]));addTable(children,'Open Ticket Aging',['Age','Count'],current.aging.map(row => [row.name,row.count])); },
