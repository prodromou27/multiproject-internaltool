...
function checkProjectAccess(req, res) {
  ...
  if (req.user.role !== 'manager' && req.user.role !== 'pm') {
    const assigned = db.prepare('SELECT 1 FROM project_assignments WHERE project_id = ? AND user_id = ?').get(id, req.user.id);
...
router.get('/', requireAuth, (req, res) => {
  const pid = checkProjectAccess(req, res);
  if (!pid) return;
  const fields = db.prepare('SELECT * FROM project_custom_fields WHERE project_id = ? ORDER BY sort_order, id').all(pid);
...
router.put('/:fid', requireAuth, (req, res) => {
  const pid = checkProjectAccess(req, res);
  if (!pid) return;
  if (!canManage(req)) return res.status(403).json({ error: 'Forbidden' });
  const f = db.prepare('SELECT * FROM project_custom_fields WHERE id = ? AND project_id = ?').get(req.params.fid, pid);
  ...
  db.prepare(`UPDATE project_custom_fields SET
...
router.delete('/:fid', requireAuth, (req, res) => {
  const pid = checkProjectAccess(req, res);
  if (!pid) return;
  if (!canManage(req)) return res.status(403).json({ error: 'Forbidden' });
  db.prepare('DELETE FROM project_custom_fields WHERE id = ? AND project_id = ?').run(req.params.fid, pid);
  res.json({ ok: true });
});

// GET /api/projects/:projectId/custom-fields/values/:taskId
router.get('/values/:taskId', requireAuth, (req, res) => {
  const pid = checkProjectAccess(req, res);
  if (!pid) return;
  // Verify the task actually belongs to this project (prevents cross-project data leak)
  const task = db.prepare('SELECT id FROM tasks WHERE id = ? AND project_id = ?').get(req.params.taskId, pid);
  if (!task) return res.status(404).json({ error: 'Task not found in this project' });
  const rows = db.prepare('SELECT field_id, value FROM task_custom_values WHERE task_id = ?').all(req.params.taskId);
  const map = {};
...
router.put('/values/:taskId', requireAuth, (req, res) => {
  const pid = checkProjectAccess(req, res);
  if (!pid) return;
  const task = db.prepare('SELECT id FROM tasks WHERE id = ? AND project_id = ?').get(req.params.taskId, pid);
  ...
  const upsert = db.prepare('INSERT OR REPLACE INTO task_custom_values (task_id, field_id, value) VALUES (?,?,?)');
  db.transaction(() => {
    for (const [fieldId, value] of Object.entries(values)) {
      upsert.run(task.id, parseInt(fieldId, 10), value !== null && value !== undefined ? String(value) : null);
    }
  })();
  res.json({ ok: true });
});
