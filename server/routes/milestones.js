...
/* ── Access helpers ──────────────────────────────────────── */
function canView(req, projectId) {
...
function canManageMilestone(req, milestone) {
...
router.get('/', requireAuth, (req, res) => {
  ...
  if (!canView(req, project_id)) return res.status(403).json({ error: 'Forbidden' });
...
router.put('/:id', requireAuth, (req, res) => {
  if (!canManage(req)) return res.status(403).json({ error: 'Forbidden' });
  const m = db.prepare('SELECT * FROM project_milestones WHERE id = ?').get(req.params.id);
  if (!m) return res.status(404).json({ error: 'Not found' });
  if (!canManageMilestone(req, m)) return res.status(403).json({ error: 'Forbidden — not assigned to this project' });
...
router.delete('/:id', requireAuth, (req, res) => {
  ...
  db.prepare('DELETE FROM project_milestones WHERE id = ?').run(m.id);
