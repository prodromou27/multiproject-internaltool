...
router.post('/:project_id', requireManager, async (req, res) => {
  ...
  const result = (await db.prepare('INSERT INTO kpis (project_id, name, target_value, current_value, unit, updated_by) VALUES (?, ?, ?, ?, ?, ?)').run(req.params.project_id, name, target_value, current_value || 0, unit || null, req.user.id));
  res.json({ id: result.lastInsertRowid });
});

router.put('/:project_id/:id', requireManager, async (req, res) => {
  const { name, target_value, current_value, unit } = req.body;
  (await db.prepare(`UPDATE kpis SET name=COALESCE(?,name), target_value=COALESCE(?,target_value),
...
router.delete('/:project_id/:id', requireManager, async (req, res) => {
  (await db.prepare('DELETE FROM kpis WHERE id = ? AND project_id = ?').run(req.params.id, req.params.project_id));
  res.json({ ok: true });
});
