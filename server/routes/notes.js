...
router.get('/note', requireAuth, (req, res) => {
  const row = db.prepare('SELECT content, updated_at FROM personal_notes WHERE user_id = ?').get(req.user.id);
  res.json(row || { content: '', updated_at: null });
});

router.put('/note', requireAuth, (req, res) => {
...
router.put('/todos/:id', requireAuth, (req, res) => {
  ...
  const todo = db.prepare('SELECT * FROM personal_todos WHERE id = ? AND user_id = ?').get(req.params.id, req.user.id);
...
router.delete('/todos/:id', requireAuth, (req, res) => {
  db.prepare('DELETE FROM personal_todos WHERE id = ? AND user_id = ?').run(req.params.id, req.user.id);
  res.json({ ok: true });
});

// Clear completed todos
router.delete('/todos', requireAuth, (req, res) => {
  db.prepare('DELETE FROM personal_todos WHERE user_id = ? AND done = 1').run(req.user.id);
  res.json({ ok: true });
});
