...
router.get('/', requireAuth, (req, res) => {
  ...
  if (task_id) {
    const task = db.prepare('SELECT assigned_to, project_id FROM tasks WHERE id = ?').get(task_id);
    if (!task) return res.status(404).json({ error: 'Task not found' });
    if (req.user.role === 'engineer') {
      if (task.assigned_to !== req.user.id)
        return res.status(403).json({ error: 'Forbidden' });
    } else if ((req.user.role === 'planner' || req.user.role === 'pm') && task.project_id) {
      const a = db.prepare('SELECT 1 FROM project_assignments WHERE project_id = ? AND user_id = ?').get(task.project_id, req.user.id);
      if (!a) return res.status(403).json({ error: 'Forbidden' });
    }
  }
  if (visit_id) {
    if (req.user.role === 'engineer') {
      const a = db.prepare('SELECT 1 FROM maintenance_visit_engineers WHERE visit_id = ? AND user_id = ?').get(visit_id, req.user.id);
      if (!a) return res.status(403).json({ error: 'Forbidden' });
    }
...
router.post('/', requireAuth, (req, res) => {
  ...
  if (req.user.role === 'engineer') {
    if (task_id) {
      const task = db.prepare('SELECT assigned_to FROM tasks WHERE id = ?').get(task_id);
