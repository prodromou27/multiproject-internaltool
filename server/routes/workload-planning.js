...
router.get('/', async (req,res) => {
  ...
  const [engineers,tasks,visits,availability] = await Promise.all([
    ...
    db.prepare("SELECT mv.id,mv.title,mv.status,mv.scheduled_date AS date,mve.user_id,'visit' AS kind,we.remaining_hours,COALESCE(we.version,0) AS estimate_version FROM maintenance_visits mv JOIN maintenance_visit_engineers mve ON mve.visit_id=mv.id JOIN users u ON u.id=mve.user_id LEFT JOIN workload_estimates we ON we.visit_id=mv.id WHERE u.role='engineer' AND u.active=1 AND mv.status IN ('scheduled','in_progress') ORDER BY mv.scheduled_date,mv.id LIMIT 5001").all(),
  ...
  res.json({ as_of: asOf, weeks, engineers: engineers.map(engineer => capacityForEngineer(engineer,weeks,[...tasks,...visits],availability)), coverage: 'Remaining task and visit effort bucketed by due/scheduled date; overdue work goes in the first week. Visit estimates are per assigned engineer. Net weekly availability must account for leave, holidays and meetings. Reports, service follow-ups and undated/future work are excluded from capacity percentages.' });
});
