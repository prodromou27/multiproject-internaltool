...
export default function CustomerAssets({ customerId,restricted=false }) {
  ...
  return <section>
    <form onSubmit={event => { event.preventDefault();setSearch(searchInput.trim());setPage(1); }} className="flex gap-8 mb-12 flex-wrap"><label htmlFor="asset-search">Search assets</label><input id="asset-search" value={searchInput} minLength={2} maxLength={100} onChange={event => setSearchInput(event.target.value)} placeholder="Name, tag, hostname, IP, serial..." style={{ maxWidth:320 }} /><button className="btn btn-ghost" disabled={!!searchInput.trim() && searchInput.trim().length<2}>Search</button>{search && <button type="button" className="btn btn-ghost" onClick={() => { setSearchInput('');setSearch('');setPage(1); }}>Clear</button>}</form>
    <div className="flex gap-8 mb-16 flex-wrap"><label htmlFor="asset-list-coverage">Coverage</label><select id="asset-list-coverage" value={coverage} onChange={event => { setCoverage(event.target.value);setPage(1); }} style={{ width:'auto' }}><option value="">All coverage</option>{COVERAGE.map(value => <option key={value} value={value}>{value==='support' ? 'Under support' : label(value)}</option>)}</select><label htmlFor="asset-list-status">Status</label><select id="asset-list-status" value={status} onChange={event => { setStatus(event.target.value);setPage(1); }} style={{ width:'auto' }}><option value="">All statuses</option>{STATUSES.map(value => <option key={value} value={value}>{label(value)}</option>)}</select><label htmlFor="asset-list-expiry">Expiry</label><select id="asset-list-expiry" value={expiry} onChange={event => { setExpiry(event.target.value);setPage(1); }} style={{ width:'auto' }}><option value="">All dates</option><option value="expired">Expired</option><option value="30">Due within 30 days</option><option value="60">Due within 60 days</option><option value="90">Due within 90 days</option></select><button className="btn btn-primary" disabled={loading || !!error} onClick={() => setEditing({ initial:null })}>Add asset</button>{!restricted && <><button className="btn btn-ghost" onClick={() => setImporting(true)}>Import</button><button className="btn btn-ghost" disabled={exporting || loading} onClick={exportAssets}>{exporting ? 'Exporting...' : 'Export Excel'}</button></>}<button className="btn btn-ghost" disabled={loading} onClick={load}>Refresh</button></div>
