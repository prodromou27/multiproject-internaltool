...
const ReportBarChart=({ title,rows }) => { const max=Math.max(1,...rows.map(row => Number(row.count)));return <div className="card" style={{ padding:16 }}><h3 style={{ fontSize:14,marginBottom:12 }}>{title}</h3><div style={{ display:'grid',gap:9 }}>{rows.map(row => <div key={row.name} style={{ display:'grid',gridTemplateColumns:'minmax(90px,1fr) minmax(90px,2fr) 32px',gap:8,alignItems:'center',fontSize:12 }}><span title={row.name} style={{ overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{row.name}</span><span style={{ height:9,background:'var(--gray-100)',borderRadius:5,overflow:'hidden' }}><span style={{ display:'block',width:`${Number(row.count)/max*100}%`,height:'100%',background:'var(--primary)',borderRadius:5 }} /></span><strong style={{ textAlign:'right' }}>{row.count}</strong></div>)}</div></div>; };
const ThroughputChart=({ trend }) => { const max=Math.max(1,...trend.points.flatMap(point => [point.created,point.resolved]));return <div className="card" style={{ padding:16,overflowX:'auto' }}><div style={{ display:'flex',justifyContent:'space-between',gap:12,marginBottom:12 }}><h3 style={{ fontSize:14 }}>Ticket throughput</h3><span className="text-muted text-sm">Blue created · Green resolved</span></div><div style={{ minWidth:Math.max(420,trend.points.length*18),height:150,display:'flex',alignItems:'end',gap:3,borderBottom:'1px solid var(--border)' }}>{trend.points.map(point => <div key={point.date} title={`${point.date}: ${point.created} created, ${point.resolved} resolved`} style={{ flex:1,height:'100%',display:'flex',gap:1,alignItems:'end' }}><span style={{ flex:1,minHeight:point.created?3:0,height:`${point.created/max*100}%`,background:'var(--primary)',borderRadius:'2px 2px 0 0' }} /><span style={{ flex:1,minHeight:point.resolved?3:0,height:`${point.resolved/max*100}%`,background:'var(--success)',borderRadius:'2px 2px 0 0' }} /></div>)}</div><div className="text-muted text-sm" style={{ display:'flex',justifyContent:'space-between',marginTop:6 }}><span>{trend.points[0]?.date}</span><span>{trend.points.at(-1)?.date}</span></div></div>; };
...
export default function ManagedCustomerReportBuilder({ customerId,range }) {
  ...
  const request=() => ({ ...range,sections,narratives,status:'draft',template_id:templateId ? Number(templateId) : null });
  ...
  async function updateWorkflow(report,action) {
    let comment='';
    if (action==='reject' || action==='reopen') {
      comment=window.prompt(action==='reject' ? 'Explain the changes required:' : 'Explain why this finalized report must be reopened:')?.trim() || '';
      if (!comment) return;
    }
    setBusy(true);setError('');
    try {
      ...
      if (failure.status===409) {
        const latest=await api.managedCustomerReportHistory(customerId).catch(() => null);
        if (latest) setHistory(latest.rows || []);
      }
  ...
  async function toggleWorkflowHistory(report) {
    ...
    setBusy(true);setError('');
    try { const result=await api.managedCustomerReportWorkflow(customerId,report.id);setWorkflowEvents(current => ({ ...current,[report.id]:result.rows || [] })); }
    catch(failure) { setError(failure.message); }
    finally { setBusy(false); }
  }
  const workflowButtons=report => <div className="flex gap-8" style={{ flexWrap:'wrap',justifyContent:'flex-end' }}>
  ...
  return <div style={{ display:'grid',gap:18 }}>
    <section className="card"><div style={{ display:'flex',justifyContent:'space-between',gap:12,alignItems:'start',marginBottom:16 }}><div><h2 style={{ fontSize:16 }}>Managed Services report</h2><p className="text-muted text-sm mt-4">Choose customer-facing content for {range.from || 'the start date'} to {range.to || 'the end date'}, then review the calculated report data.</p></div><FileText size={20} color="var(--primary)" /></div>
      ...
      <div className="info-box" style={{ marginBottom:16 }}>Generated files begin as drafts. Submit a draft for review, approve or return it, then finalize the approved version.</div>
    ...
    {preview && <section><div style={{ display:'flex',justifyContent:'space-between',gap:12,alignItems:'end',marginBottom:10 }}><div><h2 style={{ fontSize:16 }}>Report preview</h2><p className="text-muted text-sm mt-4">{preview.customer.name} {' · '} {preview.period.from} to {preview.period.to}</p></div><span className="badge badge-done">Ready</span></div>
      ...
      {preview.tickets.enabled && sections.includes('ticket_summary') && <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:12,marginBottom:16 }}><div style={{ gridColumn:'1 / -1' }}><ThroughputChart trend={preview.tickets.analytics.trend} /></div><ReportBarChart title="Open by status" rows={preview.tickets.analytics.current.statuses} /><ReportBarChart title="Open by priority" rows={preview.tickets.analytics.current.priorities} /><ReportBarChart title="Open ticket aging" rows={preview.tickets.analytics.current.aging} /></div>}
