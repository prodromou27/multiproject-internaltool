...
export default function ManagedCustomerReportBuilder({ customerId,range }) {
  const [sections,setSections]=useState(SECTIONS.map(([key]) => key)),[narratives,setNarratives]=useState({}),[templates,setTemplates]=useState([]),[templateId,setTemplateId]=useState(''),[preview,setPreview]=useState(null),[busy,setBusy]=useState(false),[error,setError]=useState('');
  useEffect(() => { const controller=new AbortController();Promise.all([api.managedReportTemplates({ signal:controller.signal }),api.managedCustomerConfiguration(customerId,{ signal:controller.signal })]).then(([result,configuration]) => { if (controller.signal.aborted) return;const active=(result.rows || []).filter(template => template.active);setTemplates(active);const selected=active.find(template => template.id===configuration.default_report_template_id);if (selected) { setTemplateId(String(selected.id));setSections(selected.sections);setNarratives(selected.default_narratives || {}); } }).catch(failure => { if (!controller.signal.aborted) setError(failure.message); });return () => controller.abort(); },[customerId]);
  ...
  async function downloadReport(format) {
    setBusy(true);setError('');
    try { const blob=await (format==='docx'?api.managedCustomerWordReport(customerId,request()):api.managedCustomerExcelReport(customerId,request())),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=`managed_services_report_${range.from}_${range.to}.${format}`;link.click();setTimeout(() => URL.revokeObjectURL(url),1000); }
    catch(failure) { setError(failure.message); }
    finally { setBusy(false); }
  }
  return <div style={{ display:'grid',gap:18 }}>
    <section className="card"><div style={{ display:'flex',justifyContent:'space-between',gap:12,alignItems:'start',marginBottom:16 }}><div><h2 style={{ fontSize:16 }}>Managed Services report</h2><p className="text-muted text-sm mt-4">Choose customer-facing content for {range.from || 'the start date'} to {range.to || 'the end date'}, then review the calculated report data.</p></div><FileText size={20} color="var(--primary)" /></div>
      ...
      <div className="flex gap-8" style={{ flexWrap:'wrap' }}><button type="button" className="btn btn-primary" disabled={busy || !sections.length} onClick={prepare}>{busy ? 'Working...' : preview ? <><RefreshCw size={14} /> Refresh Preview</> : <><FileText size={14} /> Prepare Preview</>}</button>{preview && <><button type="button" className="btn btn-ghost" disabled={busy} onClick={() => downloadReport('docx')}><Download size={14} /> Generate Word</button><button type="button" className="btn btn-ghost" disabled={busy} onClick={() => downloadReport('xlsx')}><Download size={14} /> Export Excel</button></>}</div>
    </section>
