...
export function DataTable({ columns,rows,rowKey='id',loading=false,error='',empty='No records found.',onRetry,caption }) {
  ...
  if (!rows.length) return <div className="ui-async-state"><strong>{empty}</strong></div>;
  return <div className="ui-table-frame"><table>{caption && <caption className="sr-only">{caption}</caption>}<thead><tr>{columns.map(column => <th key={column.key} className={column.numeric?'is-numeric':''}>{column.label}</th>)}</tr></thead><tbody>{rows.map(row => <tr key={typeof rowKey==='function'?rowKey(row):row[rowKey]}>{columns.map(column => <td key={column.key} data-label={column.label} className={column.numeric?'is-numeric':''}>{column.render?column.render(row):row[column.key] ?? '—'}</td>)}</tr>)}</tbody></table></div>;
}
