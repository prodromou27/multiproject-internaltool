...
function SaveDialog({ selected,definition,references,onSaved,onClose }) {
  ...
  return <Modal title={selected ? 'Update saved report' : 'Save new report'} onClose={saving ? () => {} : onClose}>
    <form onSubmit={submit}>
      ...
      {visibility==='shared' && <div className="form-row u-dfb2165">
        <fieldset><legend>Managers</legend><div className="u-972c9ba">{references.share_users.map(row => <label key={row.id} className="flex gap-6"><input type="checkbox" checked={sharedUsers.includes(row.id)} onChange={event => setSharedUsers(old => event.target.checked ? [...old,row.id] : old.filter(id => id!==row.id))} disabled={saving} />{row.name}</label>)}</div></fieldset>
        <fieldset><legend>Teams</legend><div className="u-972c9ba">{references.share_teams.map(row => <label key={row.id} className="flex gap-6"><input type="checkbox" checked={sharedTeams.includes(row.id)} onChange={event => setSharedTeams(old => event.target.checked ? [...old,row.id] : old.filter(id => id!==row.id))} disabled={saving} />{row.name}</label>)}</div></fieldset>
...
export default function ReportBuilder() {
  ...
  return <section>
    ...
    <section className="card mb-20">
      <h2 className="u-4ff818f">Report templates</h2>
    ...
    <section className="card mb-20">
      <h2 className="u-4ff818f">Saved reports</h2>
      <div className="flex gap-8 u-62da067">{references.saved.rows.map(row => <button className="btn btn-ghost btn-sm" key={row.id} onClick={() => openReport(row.id)}>{row.name} · {row.visibility}</button>)}</div>
