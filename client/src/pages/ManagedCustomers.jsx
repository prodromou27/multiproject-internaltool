...
function Work({ id,range,refresh,kind }) {
  ...
  return <><h2 style={{ fontSize:15,marginBottom:10 }}>Project delivery</h2><div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(170px,1fr))',gap:12,marginBottom:20 }}><Metric label="Active now" value={section.summary.active_now} /><Metric label="Delayed now" value={section.summary.delayed_now} /><Metric label="Upcoming deadlines" value={section.summary.upcoming_deadlines} /><Metric label="Created in period" value={section.summary.created_period} /><Metric label="Completed in period" value={section.summary.completed_period} /></div>
...
function ServiceReview({ id,range,refresh,kind }) {
  ...
  return <><h2 style={{ fontSize:15,marginBottom:10 }}>Recommendations</h2><div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(170px,1fr))',gap:12,marginBottom:20 }}><Metric label="Open now" value={section.summary.open_now} /><Metric label="Overdue now" value={section.summary.overdue_now} /><Metric label="Created in period" value={section.summary.created_period} /><Metric label="Implemented / closed" value={section.summary.completed_period} /></div><div style={{ maxWidth:480,marginBottom:20 }}><Distribution title="Recommendations by status" rows={section.statuses.map(row => ({ ...row,name:activityLabel(row.name) }))} /></div>
...
function Timeline({ id,range,refresh }) {
  ...
  if (!data) return <div className="skeleton-table"><span /><span /><span /></div>;
  const pages=Math.max(1,Math.ceil(data.total/data.page_size));
  return <><h2 style={{ fontSize:15,marginBottom:10 }}>Customer timeline · {range.from} to {range.to}</h2>{!data.rows.length ? <div className="card empty"><p>No recorded events in this period.</p></div> : <div className="card" style={{ padding:0 }}><div style={{ display:'grid' }}>{data.rows.map((event,index) => <div key={`${event.kind}-${event.source_id}`} style={{ display:'flex',flexWrap:'wrap',gap:16,padding:'14px 18px',borderBottom:index===data.rows.length-1?'none':'1px solid var(--gray-200)',alignItems:'start' }}><span className="text-muted text-sm" style={{ flex:'0 0 145px' }}>{formatDate(event.occurred_at)}</span><strong className="text-sm" style={{ flex:'0 0 160px' }}>{timelineLabels[event.kind] || activityLabel(event.kind)}</strong><div style={{ flex:'1 1 220px',minWidth:0 }}>{event.link ? event.link.startsWith('/') ? <Link to={event.link}>{event.title}</Link> : <a href={event.link} target="_blank" rel="noreferrer">{event.title} <ExternalLink size={12} /></a> : event.title}{event.reference && <div className="text-muted text-sm">{event.reference}</div>}</div></div>)}</div></div>}
