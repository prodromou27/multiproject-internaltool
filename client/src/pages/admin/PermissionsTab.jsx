...
export function PermissionsTab() {
  ...
  const RuleSelect=({ scope,owner,permissionKey,rule }) => <select aria-label={`${permissionKey} ${scope} rule`} value={rule?rule.allowed?'allow':'deny':'default'} disabled={saving===`${scope}:${owner}:${permissionKey}`} onChange={event => change(scope,owner,permissionKey,event.target.value)} style={{ minWidth:115 }}><option value="default">Default</option><option value="allow">Allow</option><option value="deny">Deny</option></select>;
  return <div style={{ display:'grid',gap:18 }}>
    ...
    <div className="card"><div className="card-header"><div><h3 style={{ fontSize:15 }}>User exceptions</h3><p className="text-muted text-sm mt-4">A user exception takes precedence over both the role default and a role override.</p></div></div>
      ...
      {selected && <div className="table-wrap"><table><thead><tr><th>Capability</th><th>Exception</th><th>Effective source</th></tr></thead><tbody>{data.definitions.map(permission => { const rule=userRule(selected.id,permission.key),role=roleRule(selected.role,permission.key),effective=rule?`User ${rule.allowed?'allow':'deny'}`:role?`Role ${role.allowed?'allow':'deny'}`:`${labelRole(selected.role)} default`;return <tr key={permission.key}><td><strong>{permission.label}</strong><div className="text-muted text-sm">{permission.group}</div></td><td><RuleSelect scope="user" owner={selected.id} permissionKey={permission.key} rule={rule} /></td><td>{effective}</td></tr>; })}</tbody></table></div>}
    </div>
  </div>;
}
