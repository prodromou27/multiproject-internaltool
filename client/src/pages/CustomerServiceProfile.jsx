...
function CustomerHealthStrip({ customerId,data,error,onSelectTab }) {
  ...
  return <div className="cs-health-strip">
    <HealthCard icon={BriefcaseBusiness} label="Active Projects" value={data.projects.active} note={`${data.projects.delayed} delayed or overdue`} state={data.projects.delayed?'danger':'default'} />
    <HealthCard icon={CheckSquare} label="Open Tasks" value={data.tasks.open} note={`${data.tasks.overdue} overdue`} state={data.tasks.overdue?'danger':'default'} />
    ...
    <HealthCard icon={CalendarDays} label="Maintenance Visits" value={data.visits.next ? fmtDate(data.visits.next.scheduled_date) : 'None scheduled'} note={data.visits.reports_pending ? `${data.visits.reports_pending} reports pending` : `${data.visits.this_year} this year`} state={data.visits.reports_pending?'warning':'default'} />
    {data.managed?.visible && <HealthCard icon={Settings2} label="Managed Services" value={managed[0]} note={managed[1]} state={data.managed.state==='sync_attention'?'danger':data.managed.state==='setup_required'?'warning':'default'} href={['active','sync_attention'].includes(data.managed.state)?`/managed-customers/${customerId}`:undefined} onClick={!['active','sync_attention'].includes(data.managed.state)?() => onSelectTab('service-configuration'):undefined} />}
...
export default function CustomerServiceProfile() {
  ...
  useEffect(() => { if (tab!=='activities') return undefined;const controller=new AbortController();Promise.all([api.users({ signal:controller.signal }),api.activityCategories({ signal:controller.signal })]).then(([users,cats]) => { if (!controller.signal.aborted) { setEngineers(users.filter(item => item.role==='engineer'));setCategories(cats); } }).catch(() => {});return () => controller.abort(); },[tab]);
