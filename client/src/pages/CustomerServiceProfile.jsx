...
export default function CustomerServiceProfile() {
  ...
  useEffect(() => { if (!saAccess.loaded) return;const requested=query.get('section');if (requested==='assets' && !operations && !operationsError) return; /* wait: the assets grant arrives with the summary */const safe=customer360Section(user,requested,saAccess.capabilities,assetsGrant);if (requested!==safe) { const params=new URLSearchParams(location.search);params.set('section',safe);navigate({ pathname:location.pathname,search:`?${params}` },{ replace:true }); } },[location.pathname,location.search,navigate,user,saAccess.capabilities,saAccess.loaded,operations,operationsError,assetsGrant]);
