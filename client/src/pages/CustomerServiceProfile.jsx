...
function CustomerHealthStrip({ customerId,data,error,onSelectTab }) {
  ...
  return <div className="cs-health-strip">
    ...
    <HealthCard icon={CheckSquare} label="Open Tasks" value={data.tasks.open} note={`${data.tasks.overdue} overdue`} state={data.tasks.overdue?'danger':'default'} onClick={() => onSelectTab('tasks',data.tasks.overdue?'overdue':'open')} />
