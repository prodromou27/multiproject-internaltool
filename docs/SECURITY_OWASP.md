...
| A03 Injection | Parameterized SQL (`?` placeholders converted to `$n`; values are never concatenated into SQL, and report definitions use registry keys rather than identifiers from the browser); a test rejects SQLite-only syntax; CI prepares every static SQL statement against PostgreSQL; upload type allowlists; CSP blocks inline scripts. |
...
| A06 Vulnerable and Outdated Components | Docker image builds from lockfiles; CI runs `npm test` and `npm audit --omit=dev` for server and client on every push and pull request. |
...
- Run `cd server && npm test` and `cd client && npm test && npm run test:e2e` (CI runs these plus the PostgreSQL checks; confirm the latest CI run on the release branch is green).
