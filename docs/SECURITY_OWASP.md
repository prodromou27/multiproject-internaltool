...
| --- | --- |
| A01 Broken Access Control | Route middleware enforces authentication and manager/planner/engineer/PM roles; project/task/customer/visit routes include ownership checks; iCal uses scoped feed tokens instead of session JWTs. |
...
| A05 Security Misconfiguration | Helmet/CSP/HSTS support, strict CORS default, health checks, Docker production env templates, required `APP_URL`, explicit reverse-proxy trust setting. |
...
| A07 Identification and Authentication Failures | Rate limiting on auth endpoints; password length and expiry policy; TOTP 2FA with replay prevention; failed/successful login events are audited. |
